...
* 'index', optional, can be "true" or "false", the field will be indexed if it's "true". Different type of fields will be indexed differently.
* 'namespace', optional, only usable when index="true", 2 or more fields can use same namespace, which means the HAS() will search terms in all of them. By default each field has different namespace.
...
Argos has HTTP interface that supports GET method, POST is not supported.
Maximium request size is 64KB, this is hardcoded and can only be changed by re-compilation.
Argos supports multiple indices at same time, each index has their own name, and operations to the index starts with prefix '/index_name'.

Argos has following command-line arguments:
...
* "text", return documents contains "text", this condition will be expanded into a serial of terms to match, using "text" analyzer.
