...
namespace json_spirit
{
    ...
    template<typename Char_type, typename Allocator_type>
    struct string_traits {
        typedef typename std::basic_string<Char_type, std::char_traits<Char_type>, Allocator_type>::const_iterator const_iterator;
    };
    
    template<typename Allocator_type>
    struct string_traits<uint32_t, Allocator_type> {
        typedef typename std::basic_string<uint32_t, std::char_traits<uint32_t>, Allocator_type> string_type;
        typedef uint32_t char_type;
        typedef typename string_type::const_iterator const_iterator;
        const_iterator make(typename string_type::const_iterator i) const {
            return const_iterator(i);
        }
        typename string_type::const_iterator conv(const_iterator i) const {
            return i;
        }
        void push_back(string_type &str, const_iterator i) const {
            str+=*i;
        }
    };
    
    template<typename Allocator_type>
    struct string_traits<wchar_t, Allocator_type> {
        typedef typename std::basic_string<wchar_t, std::char_traits<wchar_t>, Allocator_type> string_type;
    ...
    template<typename Allocator_type>
    struct string_traits<char, Allocator_type> {
        typedef typename std::basic_string<char, std::char_traits<char>, Allocator_type> string_type;
        typedef uint32_t char_type;
        typedef utf8::unchecked::iterator<typename std::basic_string<char, std::char_traits<char>, Allocator_type>::const_iterator> const_iterator;
        const_iterator make(typename string_type::const_iterator i) const {
            return const_iterator(i);
        }
        typename string_type::const_iterator conv(const_iterator i) const {
        ...
        void push_back(string_type &str, const_iterator i) {
            const_iterator i1(i);
            i1++;
    ...
    String_type add_esc_chars( const String_type& s, bool raw_utf8 )
    {
        ...
        typedef typename String_type::value_type     Char_type;
        typedef string_traits<typename String_type::value_type, typename String_type::allocator_type> traits;
        typedef typename traits::const_iterator u32iterator;
        ...
        for (u32iterator i=tr.make(s.begin()); tr.conv(i)!=end; ++i) {
            uint32_t c(*i);
            
            if( (c<256) && add_esc_char( c, result ) ) continue;
            
            if( raw_utf8 ) {
                tr.push_back(result, i);
            } else {
                if( iswprint( c ) ) {
                    tr.push_back(result, i);
                } else {
